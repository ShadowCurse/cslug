...
typedef struct {
    ...
    unsigned int curve_tex_id;
    unsigned int band_tex_id;
    Shader shader;
    unsigned int vao_id;
    unsigned int vbo_id;
...
font_t load_font(const char *ttf_path) {
    ...
    cslug_buffers buffers = {0};

    for (int cp = 0; cp < MAX_GLYPHS; cp++) {
        font.glyphs[cp].advance = 0;
        if (' ' <= cp && cp <= '~') {
            cslug_build_glyph_for_texture(&info, cp, em_scale, &buffers, &font.glyphs[cp]);
        }
    }

    free(ttf);

    font.curve_tex_id = rlLoadTexture(buffers.curves.ptr, buffers.curves.len / 4, 1,
                                      RL_PIXELFORMAT_UNCOMPRESSED_R16G16B16A16, 1);
    rlTextureParameters(font.curve_tex_id, RL_TEXTURE_MIN_FILTER, RL_TEXTURE_FILTER_NEAREST);
    rlTextureParameters(font.curve_tex_id, RL_TEXTURE_MAG_FILTER, RL_TEXTURE_FILTER_NEAREST);

    font.band_tex_id = rlLoadTexture(buffers.bands.ptr, buffers.bands.len / 4, 1,
                                     RL_PIXELFORMAT_UNCOMPRESSED_R16G16B16A16, 1);
    rlTextureParameters(font.band_tex_id, RL_TEXTURE_MIN_FILTER, RL_TEXTURE_FILTER_NEAREST);
    rlTextureParameters(font.band_tex_id, RL_TEXTURE_MAG_FILTER, RL_TEXTURE_FILTER_NEAREST);

    cslug_free_buffers(&info, &buffers);

    font.shader = LoadShader("cslug_texture_vert.glsl", "cslug_texture_frag.glsl");
...
void free_font(font_t *font) {
    ...
    rlUnloadTexture(font->curve_tex_id);
    rlUnloadTexture(font->band_tex_id);
...
void draw_text(font_t *font, const char *text, Vector3 pos, float size, Color color, Matrix view_proj) {
    ...
    rlEnableTexture(font->band_tex_id);
    rlActiveTextureSlot(0);
    rlEnableTexture(font->curve_tex_id);
